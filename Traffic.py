...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        # creates contours
        contours, hierarchy = cv2.findContours(bins, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        ...
        #Contours in rectangle shape
        for contour in contours:
            (x, y, w, h) = cv2.boundingRect(contour)  # boundingRect gives x,y,w,h
        ...
        lineypos3 = 247
        cv2.line(image, (0, lineypos3), (width, lineypos3), (0, 255, 255), 1)
            ...
            if hierarchy[0, i, 3] == -1:  # using hierarchy to only count parent contours (contours not within others)
                ...
                if minarea < area < maxarea:  # area threshold for contour
                    ...
                    if cy > lineypos:  # filters out contours that are above line (y starts at top)
                        ...
                        if cy < lineypos2 and cy>lineypos3:
                            print(cy)
                            if (area >= 1200 and area <= 1600):
                                hatchback += 1
                        ...
                        cv2.putText(image, str(cx) + "," + str(cy), (cx + 10, cy + 10), cv2.FONT_HERSHEY_SIMPLEX,
                                    .3, (0, 0, 255), 1)
                        cv2.putText(image,str(area),(cx+10,cy+20),cv2.FONT_HERSHEY_SIMPLEX,0.3,(0,255,255),1)
            ...
            if curcent:  # if there is a current centroid
                ...
                if oldcent:  # checks if old centroid exists
                    ...
                    # to count cars and that car hasn't been counted yet
                        ...
                        cv2.line(image, (0, lineypos2), (width, lineypos2), (255, 255, 125), 5)
        ...
        cv2.putText(image,"Hacthbacks : "+str(hatchback),(0,105),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,170,0),1)
        cv2.putText(image, "Sedans : " + str(sedan), (0, 125), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
        cv2.putText(image, "Suv : " + str(suv), (0, 145), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0), 1)
        ...
        k = cv2.waitKey(int(4500/fps)) & 0xff  # int(1000/fps) is normal speed since waitkey is in ms
        if k == 27:
            break
